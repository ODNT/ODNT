...
void Serveur::errorSocket(QAbstractSocket::SocketError error)
{
	switch(error)
	{
		case QAbstractSocket::HostNotFoundError:
            affichage->append(tr("<em>ERROR : can't find olddogsnewtricks server.</em>"));
			break;
...
{
    affichage->append("Connecting... to olddogsnewtricks IRC. Please wait.");

	sendData("USER "+pseudo+" localhost "+serveur+" :"+pseudo);
    sendData("NICK "+pseudo);
    affichage->append("Connected to olddogsnewtricks IRC.");

}

void Serveur::joins()
{
    join("#olddogsnewtricks");
}
